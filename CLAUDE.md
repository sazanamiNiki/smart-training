...
| リソース | 役割 |
|---------|------|
| **Cloudflare Worker** | APIエンドポイント（OAuth・提出・レビュー取得） |
| **D1（SQLite）** | 提出履歴・集計レビューの永続化 |
| **R2（Object Storage）** | Claude生成レビュー本文（Markdown）の保存 |

- D1 はリレーショナルデータ（誰が・いつ・どの問題を提出したか）の管理に使用
- R2 はレビュー本文（長文Markdown）のオブジェクトストレージとして使用
- Worker から D1・R2 へはバインディング経由でアクセスする（環境変数不要）
...
CREATE TABLE submissions (
  ...
  user_id     TEXT    NOT NULL,          -- GitHub ユーザーID
  qu_id       TEXT    NOT NULL,          -- 問題ID（例: qu1）
  code        TEXT    NOT NULL,          -- 提出コード
  submitted_at TEXT   NOT NULL           -- ISO8601 タイムスタンプ
);
```

#### `aggregate_reviews` テーブル

```sql
CREATE TABLE aggregate_reviews (
  qu_id        TEXT    PRIMARY KEY,      -- 問題ID（例: qu1）
...
| キー | 内容 |
|-----|------|
| `reviews/{user_id}/{qu_id}/review.md` | 個人レビュー（提出時にClaudeが生成） |
| `aggregate/{qu_id}/review.md` | 集計レビュー（5件達成時にClaudeが生成） |

### エンドポイント一覧（Cloudflare Worker）

| メソッド | パス | 説明 |
|--------|------|------|
| `POST` | `/submit` | コード提出・個人レビュー生成・D1記録 |
| `GET` | `/mypage` | ユーザーの提出履歴・個人レビュー一覧取得 |
| `GET` | `/review?qu_id={qu_id}` | 指定問題の集計レビュー取得 |
| `GET` | `/github/callback` | GitHub OAuth コールバック（既存） |

#### `POST /submit` の拡張内容

従来の GitHub コミット処理に加え、以下を実行する：

1. D1 の `submissions` に提出データを INSERT
2. Claude API を呼び出してコードレビューを生成
3. 生成レビューを R2 の `reviews/{user_id}/{qu_id}/review.md` に保存
4. 提出件数が5件に達した場合、集計レビューを生成（後述）
5. Slack に完了通知を送信（既存）

### レビュー生成フロー

```
ユーザーがコード提出（POST /submit）
  │
  ├─ D1: submissions に INSERT
  ├─ Claude API: 個人レビューを生成（コード品質・改善提案）
  ├─ R2: reviews/{user_id}/{qu_id}/review.md に保存
  │
  ├─ D1: submissions の件数を集計（同 qu_id）
  │   └─ 5件以上 かつ aggregate_reviews に未登録 → 集計レビュー生成へ
  │       ├─ D1: 直近5件のコードを取得
  │       ├─ Claude API: 集計レビューを生成（傾向分析・共通パターン）
  │       ├─ R2: aggregate/{qu_id}/review.md に保存
  │       └─ D1: aggregate_reviews に INSERT（冪等チェック済み）
  │
  └─ Slack: 提出完了通知
```
...
- **トリガー**: 同一 `qu_id` の提出が合計5件に達したとき
- **冪等チェック**: `aggregate_reviews` テーブルに該当 `qu_id` が存在する場合はスキップ（再生成しない）
- **再生成**: 10件・15件…と5件ごとに `aggregate_reviews` の `generated_at` を更新して再生成する
  - 判定式: `submissions 件数 % 5 === 0` かつ `aggregate_reviews.generated_at < 最新提出日時`
