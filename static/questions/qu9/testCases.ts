...
export const testCases = [
  {
    ...
    input: [DATA_BASIC.words, DATA_BASIC.maxWidth],
    expected: DATA_BASIC.expected
  },
  {
    name: '正常系: スペースが割り切れない場合（左側の隙間を広くする）',
    input: [DATA_UNEVEN_SPACES.words, DATA_UNEVEN_SPACES.maxWidth],
    expected: DATA_UNEVEN_SPACES.expected
  },
  {
    name: '正常系: 1行に1単語のみの場合（右側をスペースで埋める）',
    input: [DATA_SINGLE_WORDS.words, DATA_SINGLE_WORDS.maxWidth],
    expected: DATA_SINGLE_WORDS.expected
  },
  {
    name: '正常系: 最終行に複数単語がある場合（左寄せ・単語間スペース1）',
    input: [DATA_LAST_LINE_MULTI.words, DATA_LAST_LINE_MULTI.maxWidth],
    expected: DATA_LAST_LINE_MULTI.expected
  },
  {
    name: '異常系: 単語の長さがmaxWidthを超えている場合',
    input: [DATA_ERROR_OVER_LENGTH.words, DATA_ERROR_OVER_LENGTH.maxWidth],
    expectedError: true,
    expected: `Word Hello (length 5) exceeds maxWidth (3)`
  }
];
