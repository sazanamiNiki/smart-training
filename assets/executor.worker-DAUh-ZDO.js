...
`;self.onmessage=async e=>{try{const{type:n,code:r,constants:f}=e.data;if(n==="execute"){const I=ge(await ue(r));let S="";f&&(S=ge(await ue(f)));const V=[kt,I,S,"return __consoleLogs;"].join(`
`),x=new Function(V)();self.postMessage({type:"console-result",logs:x});return}if(n!=="run")return;const{testCode:g,testCases:p}=e.data,h=ge(await ue(r));let b="";f&&(b=ge(await ue(f)));const d=_t(await ue(g)),k=[xt,h,b,`var testCases = ${JSON.stringify(p)};`,d,"return __testResults;"].join(`
